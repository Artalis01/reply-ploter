...
def get_topic(sb):
    ...
    # tweet_details = cell_inner_div.find_elements(By.XPATH, ".//div[contains(@aria-label, 'reposts')]")
    ...
    # tweet_detail = [element.get_attribute('aria-label') for element in tweet_details]

    # print("URL".ljust(10, "=")+">", tweet_url)
    print("Username".ljust(10, "=")+">", tweet_user)
    # print("Detail".ljust(10, "=")+">", tweet_detail)
    print("Content".ljust(10, "=")+">", tweet_text)

    if len(tweet_url)>0 and len(tweet_text)>0: # and len(tweet_detail)>0:
        ...
        # tweet['replies'] = tweet_detail[0]
        # tweet['reposts'] = tweet_detail[1]
        # tweet['likes'] = tweet_detail[2]
        # tweet['views'] = tweet_detail[3]
        return tweet
...
def scroll_replies(sb, status, tweet_replies, scrollPixels = 2450):
    ...
    while True:
        ...
        status.update(label=total_data)
    ...
    status.update(label=total_data, expanded = False)

def login(sb, username, password, email):
    sb.wait(3)
    if not sb.is_element_present('a[data-testid="login"]'):
        return None
    sb.click('a[data-testid="login"]')
    sb.wait(3)
    sb.type('input[autocomplete="username"]', username)
    sb.click('button:contains("Next")')
    sb.wait(3)
    if sb.is_element_present('input[data-testid="ocfEnterTextTextInput"]'):
        sb.type('input[data-testid="ocfEnterTextTextInput"]', email)
        sb.click('button:contains("Next")')
...
def get_replies_data(sb, status, tweet_replies):
    # return all replies from a tweet
    start_time = t.time()

    scroll_replies(sb, status, tweet_replies)

    end_time = t.time()
    time_spent = end_time - start_time
    print("Time spent for a single tweet:", round(time_spent), "seconds")
    # minutes = round(time_spent/60, 2)
    # print("Total time spent: ", minutes, "minutes")

def xcrawl(status, url):
    if url == '':
        st.info('Masukkan link URL pada kolom yang disediakan di atas')
        return [],[]
    ...
    with SB(headless=True, user_data_dir='/workspaces/reply-ploter/user/profile1') as sb:
        sb.open(url)
        status.update(label="Sedang mempersiapkan data. . .")
        login(sb, xp.username, xp.password, xp.email)

        status.update(label="(1/4) Mengambil topik tweet. . .")
        topic = get_topic(sb)

        status.update(label="(2/4) Mengambil reply tweet. . .")
        get_replies_data(sb, status, tweet_replies)
